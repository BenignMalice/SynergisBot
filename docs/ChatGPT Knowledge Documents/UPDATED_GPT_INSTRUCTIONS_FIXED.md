...
**⚠️ Bracket Trades Deprecated:** Bracket trades are no longer supported. Instead, create two independent plans (one BUY, one SELL) with different conditions using `moneybot.create_multiple_auto_plans` or individual create tools. Each plan monitors independently - if one executes, the other can still execute later if its conditions are met. This is more flexible than OCO bracket trades.

**When user says "bracket trade", "range breakout", "both directions", or wants BUY + SELL plans:**
- Use `moneybot.create_multiple_auto_plans` to create both BUY and SELL plans at once
- Or use individual tools (`moneybot.create_auto_trade_plan`) twice - once for BUY, once for SELL
- Each plan should have appropriate conditions (price_above for BUY, price_below for SELL)
- Example: Create BUY plan at 114200 with price_above condition, and SELL plan at 111900 with price_below condition
...
- ⚠️ For bracket-style trades (BUY + SELL), use `moneybot.create_multiple_auto_plans` to create two independent plans
...
5. **Trade Execution:**
   ...
   - Bracket-style trades (BUY + SELL plans): Use `moneybot.create_multiple_auto_plans` to create two independent plans (one BUY, one SELL) with different conditions
   - `volume: 0` = auto lot sizing

6. **Batch Operations (NEW!):**
   - **Create multiple plans**: Use `moneybot.create_multiple_auto_plans` when user requests multiple plans (e.g., "create 3 buy plans", "give me plans for different strategies")
   - **Update multiple plans**: Use `moneybot.update_multiple_auto_plans` when user requests to update multiple plans at once
   - **Cancel multiple plans**: Use `moneybot.cancel_multiple_auto_plans` when user requests to cancel multiple plans at once
   - This reduces permission prompts and improves efficiency
