...
class IndicatorBridge:
    ...
    def _calculate_indicators(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            # Additional indicators: MACD (12,26,9), Stochastic (14,3), OBV
            try:
                # MACD
                ema12_series = pd.Series(close).ewm(span=12, adjust=False).mean()
                ema26_series = pd.Series(close).ewm(span=26, adjust=False).mean()
                macd_line_series = ema12_series - ema26_series
                macd_signal_series = macd_line_series.ewm(span=9, adjust=False).mean()
                macd_line = float(macd_line_series.iloc[-1]) if len(macd_line_series) > 0 else 0.0
                macd_signal = float(macd_signal_series.iloc[-1]) if len(macd_signal_series) > 0 else 0.0
                macd_hist = macd_line - macd_signal

                # Stochastic Oscillator
                lowest_low_series = pd.Series(low).rolling(14).min()
                highest_high_series = pd.Series(high).rolling(14).max()
                stoch_k_series = ((pd.Series(close) - lowest_low_series) /
                                  (highest_high_series - lowest_low_series)) * 100
                stoch_k = float(stoch_k_series.iloc[-1]) if len(stoch_k_series) > 0 else 0.0
                stoch_d_series = stoch_k_series.rolling(3).mean()
                stoch_d = float(stoch_d_series.iloc[-1]) if len(stoch_d_series) > 0 else 0.0
                ...
                logger.error(f"Error calculating additional indicators: {ex}", exc_info=True)
            ...
            return {
                ...
                # New indicators (not rounded for histogram precision)
                "macd_line": macd_line,
