...
# --- helpers for VirtualPendingManager interop ----------------------------



# === Phase3 helpers: normalize pending templates from strategy_plan ===
def _build_pending_from_plan(plan: dict, tech: dict) -> dict:
    """Return a dict for virt_pendings.add(...) from a strategy_plan.

    Supports two shapes:
      - Breakout retest: {pending_type: BUY_STOP/SELL_STOP, retest_level, buffer, sl, tp, lot?, expiry_ts?}
      - Mean reversion: {pending_type: BUY_LIMIT/SELL_LIMIT, mean_kind: EMA20|VWAP|Pivot, sl, tp, entry?}
    """
    out = {}
    ptype = str(plan.get("pending_type") or "").upper()
    side = "buy" if "BUY" in ptype else "sell"
    out["symbol"] = str(plan.get("symbol") or tech.get("symbol") or plan.get("sym") or "")
    out["side"] = side
    out["ptype"] = ptype
    if ptype in ("BUY_STOP","SELL_STOP","OCO_STOP"):
        out["style"] = "breakout"
        ret = float(plan.get("retest_level") or plan.get("retest") or 0.0)
        buf = float(plan.get("buffer") or plan.get("buffer_pts") or 0.0)
        ent = plan.get("entry") or (ret + buf if "BUY" in ptype else ret - buf)
        out["retest_level"] = ret
        out["buffer_pts"] = buf
        out["entry"] = float(ent)
        out["sl"] = float(plan.get("sl"))
        out["tp"] = float(plan.get("tp"))
    elif ptype in ("BUY_LIMIT","SELL_LIMIT"):
        out["style"] = "reversion"
        out["mean_kind"] = str(plan.get("mean_kind") or "EMA20")
        out["entry"] = float(plan.get("entry") or plan.get("mean_level") or 0.0)
        out["sl"] = float(plan.get("sl"))
        out["tp"] = float(plan.get("tp"))
    else:
        raise ValueError(f"Unsupported pending_type: {ptype}")
    if "lot" in plan:
        out["lot"] = float(plan["lot"])
    if "expiry_ts" in plan:
        out["expiry_ts"] = int(plan["expiry_ts"])
    # attach regime/ADX for audit
    try:
        out["regime"] = str(tech.get("regime") or tech.get("market_regime") or "")
    except Exception:
        pass
    try:
        out["adx_m15_at_plan"] = float(tech.get("_tf_M15", {}).get("adx") or tech.get("adx") or 0.0)
    except Exception:
        pass
    return out


def _ensure_task_obj(pm, maybe_task):
    """
    Some older paths return an int (row id). New manager returns a VPendingTask.
    This normalises it to a task object.
    """
    if isinstance(maybe_task, int):
        return pm.get(int(maybe_task))
    return maybe_task
...
def _normalize_expiry_ts(payload: dict, default_ttl_minutes: int = 60) -> int:
    """Return integer epoch seconds; accepts 'expiry_ts' or legacy 'expires_ts'."""
    try:
        if payload.get("expiry_ts") is not None:
            return int(float(payload["expiry_ts"]))
        if payload.get("expires_ts") is not None:  # legacy
            return int(float(payload["expires_ts"]))
    except Exception:
        pass
    return int(time.time()) + int(default_ttl_minutes * 60)
...
def _build_pending_message(
    sym: str, plan: dict, expiry_ts: Optional[int], rr_floor: float, show_cooloff: bool
) -> str:
    ...
    exp = (
        f" (expires {datetime.fromtimestamp(expiry_ts).strftime('%H:%M')})"
        if expiry_ts
        else ""
    )
    parts = [
    ...
    try:
        ...
        if fakeout_ok and prefer_side:
            ...
            if fade and protect:
                ...
                # FIXED: Calculate SL/TP for each leg using ATR-based positioning
                atr = float(tech.get("atr_14", 0.0))
                if atr > 0:
                    for leg in (fade, protect):
                        ...
                        # SL: 1.2× ATR away from entry, on the losing side
                        if "BUY" in leg_type:
                            leg["sl"] = entry - (1.2 * atr)
                            leg["tp"] = entry + (2.0 * (entry - leg["sl"]))  # 2.0 RR
                        else:  # SELL
                            leg["sl"] = entry + (1.2 * atr)
                            leg["tp"] = entry - (2.0 * (leg["sl"] - entry))  # 2.0 RR
                        
                        leg["rr"] = _calc_rr(leg.get("entry"), leg.get("sl"), leg.get("tp"))
                else:
                    # Fallback: inherit from primary plan if ATR unavailable
                    for leg in (fade, protect):
        ...
        expiry_ts = int(time.time() + ttl_min * 60)
        ...
        try:
            ...
            if blocked:
                text = _build_pending_message(
                    sym, plan, expiry_ts, rr_floor, show_cooloff
                )
                text += (
        ...
        text = _build_pending_message(sym, plan, expiry_ts, rr_floor, show_cooloff)
        key = ("pending_last", chat_id, sym)
        context.application.bot_data[key] = {"plan": plan, "expiry_ts": expiry_ts}
...
async def pendings_list_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = []
    lines = ["**Pendings (OPEN/ARMED):**"]
    # Build per-symbol buckets
    by_sym = {}
    for t in tasks:
    ...
    for sym, items in by_sym.items():
        items = sorted(items, key=lambda x: int(getattr(x, "id", 0)))
        for t in items:
            st = str(getattr(t, "status", "") or "").upper()
            ptype = str(getattr(t, "pending_type", "") or "").replace("_", " ").upper()
            entry_s = fmt_price(t.symbol, t.entry)
            sl_s = fmt_price(t.symbol, t.sl) if t.sl is not None else "—"
            tp_s = fmt_price(t.symbol, t.tp) if t.tp is not None else "—"
            lines.append(
                f"- [{st}] {t.symbol} {ptype} @ {entry_s} (SL {sl_s}, TP {tp_s}) • id={t.id}"
            )
            rows.append(
                [
                    InlineKeyboardButton(
    ...
    # Add per-symbol cancel-all buttons
    for sym in sorted(by_sym.keys()):
        rows.append(
            [
                InlineKeyboardButton(
                    f"❌ Cancel all {sym}", callback_data=f"pend|cancel_sym|{sym}"
                )
            ]
        )
...
def _pm_add_compat(pm, **kwargs):
    """Compatibility wrapper for VirtualPendingManager.add(**kwargs)
    - Guarantees we have an expiry timestamp
    - Canonicalizes ptype (spaces->underscores)
    - Adapts to managers expecting 'expires_ts'
    - Falls back volume->lot if needed
    - **New:** If manager returns an int (rowid), coerce to an object with attributes used by callers.
    """
    # 1) Normalize expiry
    try:
        if "expiry_ts" not in kwargs or kwargs.get("expiry_ts") in (None, ""):
            kwargs["expiry_ts"] = _normalize_expiry_ts(kwargs)
    except Exception:
        pass
    # Remove legacy duplicate
    kwargs.pop("expires_ts", None)

    # 2) Canonicalize pending type
    try:
        if "ptype" in kwargs and kwargs["ptype"]:
            kwargs["ptype"] = str(kwargs["ptype"]).strip().upper().replace(" ", "_")
    except Exception:
        pass

    # 3) Detect manager signature and adapt field name if needed
    try:
        sig = inspect.signature(pm.add)
        if "expiry_ts" not in sig.parameters and "expires_ts" in sig.parameters:
            kwargs["expires_ts"] = kwargs.pop("expiry_ts", None)
    except Exception:
        pass
    ...
    try:
        rv = pm.add(**kwargs)
        # If the manager returns an integer ID, turn it into a richer task object
        if isinstance(rv, int):
            task_id = rv
            # Preferred: manager can fetch by id
            task = None
            if hasattr(pm, "get"):
                try:
                    task = pm.get(task_id)
                    ...
                    task = None
            if task is None:
                # Fallback: search list_for_chat
                chat_id = kwargs.get("chat_id")
                if chat_id is not None and hasattr(pm, "list_for_chat"):
                    try:
                        tasks = pm.list_for_chat(int(chat_id)) or []
                        for t in tasks:
                        ...
                        task = None
            if task is None:
                # Last resort: synthesize a minimal object with expected attributes
                class _T:
                    pass

                t = _T()
                t.id = task_id
                t.chat_id = int(kwargs.get("chat_id") or 0)
                t.symbol = kwargs.get("symbol")
                t.side = str(kwargs.get("side") or "").lower()
                t.pending_type = kwargs.get("pending_type") or kwargs.get("ptype")
                t.entry = (
                    float(kwargs.get("entry"))
                    if kwargs.get("entry") is not None
                    else None
                )
                t.sl = None if kwargs.get("sl") is None else float(kwargs.get("sl"))
                t.tp = None if kwargs.get("tp") is None else float(kwargs.get("tp"))
                v = kwargs.get("volume", kwargs.get("lot"))
                try:
                    t.volume = float(v) if v is not None else None
                except Exception:
                    t.volume = None
                task = t
            return task
        ...
        msg = str(e)
        # manager doesn't accept 'volume': map to 'lot'

        if "unexpected keyword argument 'volume'" in msg and "volume" in kwargs:
            v = kwargs.pop("volume")
            kwargs["lot"] = v
            return pm.add(**kwargs)
        # manager rejects expiry_ts but accepts expires_ts
        if "unexpected keyword argument 'expiry_ts'" in msg:
            val = kwargs.pop("expiry_ts", None)
            if val is not None:
                kwargs["expires_ts"] = val
            return pm.add(**kwargs)
        raise
    ...
    try:
        ...
        if action == "cancel_all":
            ...
            if pm is None:
                ...
                return
            armed = pm.list_for_chat(chat_id)
            n = 0
            for t in armed:
                if pm.cancel(t.id, chat_id=chat_id):
                    n += 1
        ...
        if action == "cancel_sym":
            ...
            if pm is None:
                ...
                return
            armed = pm.list_for_chat(chat_id) or []
            n = 0
            for t in armed:
                try:
                    if str(getattr(t, "symbol", "")).upper() == str(_sym or "").upper():
                        if pm.cancel(t.id, chat_id=chat_id):
                            n += 1
                except Exception:
                    continue
            m = await query.message.reply_text(
                f"🗑 Cancelled {n} pending(s) for {(_sym or '').upper()}."
            )
            try:
                from handlers.menu import main_menu_markup
        ...
        if action == "cancel":
            ...
            if pm is None:
                ...
                return
            ok = pm.cancel(_id, chat_id=chat_id)
            m = await query.message.reply_text(
        ...
        if action == "armk":
            ...
            required = ["symbol", "side", "ptype", "entry", "lot", "expiry_ts"]
            ...
            # IMPROVED: Safety check - NEVER arm a pending order without valid SL/TP
            sl_val = payload.get("sl")
            tp_val = payload.get("tp")
            if sl_val is None or sl_val == 0 or sl_val == 0.0 or sl_val == "0":
                logger.error(
                    f"CRITICAL: Refusing to arm pending order without SL. payload={_debug_payload_preview(payload)}"
                )
                await query.message.reply_text(
                    "⚠️ SAFETY BLOCK: Cannot arm pending order without a valid Stop Loss (SL). "
                    "Trading without stops is extremely dangerous. Please run /pending again with proper risk management."
                )
                return
            if tp_val is None or tp_val == 0 or tp_val == 0.0 or tp_val == "0":
                logger.warning(
                    f"WARNING: Arming pending order without TP. sl={sl_val}, payload={_debug_payload_preview(payload)}"
                )
                # Allow TP=None (some traders manage exits manually), but warn
            ...
            m = await query.message.reply_text(
                ...
                f"Expires at {datetime.fromtimestamp(int(payload['expiry_ts'])).strftime('%H:%M')}.\n"
        ...
        if action == "arm_oco":
            ...
            expiry_ts = int(payload.get("expiry_ts") or (time.time() + 60 * 60))
